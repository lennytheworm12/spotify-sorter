...
root=Path.cwd();run=root/'reports/stage5g1b_taxonomy_review/v1'
output=Path(args.output_dir);output.mkdir(parents=True,exist_ok=True)
url=args.url
with urllib.request.urlopen(url+'/api/ping') as response:
    assert json.load(response)['mode']=='taxonomy_audit_disposable','Refusing to write test answers to the real reviewer'
with urllib.request.urlopen(url+'/api/session') as response:session=json.load(response)
assert session['completed']==0 and session['total']==16
def keys(value):
...
with sync_playwright() as p:
    ...
    assert len(rows)==16 and all(r['save_status']=='SAVED' for r in rows)
