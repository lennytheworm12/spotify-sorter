...
def _prepare(tmp_path: Path) -> Path:
    ...
    # Synthetic normalized vectors exercise analysis without a private local cache.
    rng = np.random.default_rng(512)
    rows = []
    for index, source in enumerate(source_tracks):
        row = {"stable_track_id": source["spotify_track_id"], "status": "SUCCESS",
               "representation_identity": f"fixture-representation-{index}",
               "source_audio_sha256": f"{index:064x}"}
        for encoder, dimension in (("clap", 512), ("muq", 1024)):
            vector = rng.normal(size=dimension).astype(np.float32)
            vector /= np.linalg.norm(vector)
            row[f"{encoder}_embedding"] = vector.tolist()
            row[f"{encoder}_embedding_sha256"] = f"{encoder}-{index}"
        rows.append(row)
    pq.write_table(pa.Table.from_pylist(rows), target_dataset / "part-fixture.parquet")
    return tmp_path
