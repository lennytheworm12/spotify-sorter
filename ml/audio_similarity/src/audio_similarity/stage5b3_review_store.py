"""Autosaving review store for Stage 5B.3 changed selections."""
...
from .stage5b3_minimal_selector import LABELS, REVIEW_COLUMNS, REVIEW_SCHEMA_VERSION


MAX_NOTE_LENGTH = 2_000


class Stage5B3ReviewStore:
    """Expose the targeted CSV through the established review-workbench API."""
    ...
    def _read_rows(self) -> list[dict[str, str]]:
        with self.review_path.open(encoding="utf-8", newline="") as handle:
            ...
            rows = list(reader)
        if not rows:
            raise Stage5B1AValidationError("Stage 5B.3 review queue is empty")
        ...
        for row in rows:
            ...
            if identity in identities or not YOUTUBE_VIDEO_ID.fullmatch(identity[1]):
                raise Stage5B1AValidationError("invalid Stage 5B.3 review identity")
            identities.add(identity)
            if row["review_schema_version"] != REVIEW_SCHEMA_VERSION:
                raise Stage5B1AValidationError("unexpected Stage 5B.3 review schema")
            label = row["candidate_review_label"].strip().upper()
            if label and label not in LABELS:
                raise Stage5B1AValidationError("invalid Stage 5B.3 review label")
            row["candidate_review_label"] = label
            if len(row["candidate_note"]) > MAX_NOTE_LENGTH or len(row["track_note"]) > MAX_NOTE_LENGTH:
                raise Stage5B1AValidationError("Stage 5B.3 review note is too long")
        return rows
    ...
    def session(self) -> dict[str, Any]:
        with self._lock:
            ...
            cases = [{
                ...
                "candidates": [{
                    "display_index": int(row["selected_rank"]),
            ...
            return {
                "schema_version": "stage5b3-minimal-selector-review-session-v1",
    ...
    def submit(
        ...
        with self._lock:
            ...
            if target is None:
                raise Stage5B1AValidationError("unknown Stage 5B.3 review identity")
