    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        if (!['GET', 'HEAD'].includes(req.method ?? ''))
          return send(405, { message: 'Read-only endpoint.' })
        const host = req.headers.host ?? ''
        if (!/^(127\.0\.0\.1|localhost|\[::1\])(:\d+)?$/.test(host))
          return send(403, { message: 'Local access only.' })
        try {
          if (req.headers.origin && new URL(req.headers.origin).host !== host)
            return send(403, { message: 'Same-origin access only.' })
        } catch {
        ...
        try {
          ...
          const file = await realpath(index[audio[1]].path),
            mediaRoot = await realpath(mediaDirectory)
          const pathWithinRoot = relative(mediaRoot, file)
          if (
            pathWithinRoot.startsWith('..') ||
            pathWithinRoot.startsWith('/') ||
            !/\.(webm|mp3|m4a|ogg|opus|wav|flac|aac)$/i.test(file)
          )
            return send(403, { message: 'Invalid media path.' })
          ...
          try {
            ...
            return send(416, { message: 'Invalid audio range.' })
          }
          ...
          send(503, {
            message: 'Local map files are unavailable. Run the exporter or open a local map file.',
          })
        }
      })
    },
